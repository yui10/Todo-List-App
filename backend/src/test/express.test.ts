...
describe('Express API', () => {
    ...
    test("GET /api/task", async () => {
        ...
        let tasks = [];
        for (let id of ids) {
            let task = new Task(id, dayjs(), "content", new Date().toString(), false);
            tasks.push(task);
        }
        ...
        for (let i = 0; i < res_object.length; i++) {
            res_task.push(new Task(res_object[i].id, dayjs(res_object[i].created_at), res_object[i].content, res_object[i].due_date, res_object[i].completed));
        }
    ...
    test("POST /api/task", async () => {
        let task = new Task(uuid(), dayjs(), "content", new Date().toString(), false);
        ...
        let res_object = JSON.parse(response.text);
        let res_task = new Task(res_object.id, dayjs(res_object.created_at), res_object.content, res_object.due_date, res_object.completed);
